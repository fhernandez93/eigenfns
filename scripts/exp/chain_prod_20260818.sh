...
until grep -q "GPU chain done" results/exp/chain_gpu_20260818.log 2>/dev/null; do
  sleep 180
done
echo "=== $(date) I1 chain done; launching N=10k production S_below"
...
echo "=== $(date) production chain done"
