...
z1 = np.load(SRC / "dos_n1k_128.npz")
ev_c = np.load(RES / "i4_n1000_circ_G128" / "eigenvalues_all.npy")
lo, hi = 1.60, 2.30
m = (z1["lam"] >= lo) & (z1["lam"] <= hi)
axc.fill_between(z1["lam"][m], 1, np.maximum(z1["rho"][m], 1), color=LGREY, lw=0)
axc.plot(z1["lam"][m], np.maximum(z1["rho"][m], 1), color=GREY, lw=0.8, label=r"KPM DOS ($128^3$, $d=8000$)")
...
axd.vlines(lam10[~ing], 1, 3.5, color=BLUE, lw=0.6, label=f"certified eigenvalues ({(~ing).sum()})")
