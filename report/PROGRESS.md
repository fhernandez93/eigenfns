...
## 9. Post-delivery maintenance (2026-09-04)
- Repository reorganised: project records under `docs/` (`REPORT.md` → `docs/REPORT_N1000.md`, `plans/` → `docs/plans/`, kickoff prompts → `docs/kickoff/`); every quoted path rewritten in the scripts, the SM and the ledgers. No number or figure changed.
- `results/prod_N1000_G128` moved back from the external drive into the repo tree (rsync, checksum-verified, 220 files) and its 20 GB of `solve_block*` checkpoints deleted (`PRUNED.md` there). Gate G6 loads those blocks and is no longer re-runnable on this directory without a 5.5 h re-solve; its 2026-08-14 verdict stands in `results/gates/gate_results.json`. `common.py`: `USB` → `PROD_N1K = RES / "prod_N1000_G128"`.
- `bash build.sh --figures` re-run from the relocated data: all 20 figure PNGs byte-identical to the committed ones; both PDFs at 5 / 36 pages. The regeneration exposed two reproducibility gaps, both fixed: (i) `s07_text_numbers` was missing from the `build.sh` script list, so `--figures` never regenerated its ledger; (ii) `n10k_shell_deficit_pct_160` (quoted in SM §"Limitations" as the 9.6 % lower end of the seam range) had been hand-patched into `s03_structure.json` under the label "addendum via s07", and no script produced it — `s03_structure.py` now computes it from the 160³ raster it already builds (9.6074 %, identical to the patched value). Ledger: 505 entries; against the previous commit only the source-path strings and that entry's `script` field differ.
